...
public class Axe : WeaponBase
{
    ...
    private IEnumerator AxeAltAttackCoroutine()
    {
        ...
        foreach (var axeHit in axeHits)
        {
            ...
            if (enemy != null)
            {
                ...
                float finalDamage = ((axeDamage + Manager.Instance.rb.velocity.magnitude * 5) * multiplier) + bonus + charge;
                charge = 0;
    ...
    public override string GetAltAmmoText() => bonus > 0 ? $"{Mathf.Round(Manager.Instance.rb.velocity.magnitude * 5 * axeAltDamageMultiplier)}+{bonus+charge}" : $"0+{charge}";
}